...
4. Decide panel and models pins (sections 6): run `make panel-bump TAG=<TAG>` or `make refresh-models` only after reading their diffs.
...
- drop: `git rebase -i` is not available; first `git branch -f locked-prev HEAD` (so rollback is `git reset --hard locked-prev`), then `git rebase --onto <parent-of-patch> <patch> locked`, remove the YAML entry, `make patches`, `make verify`.
